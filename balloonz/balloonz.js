...
(function () {
	...
	setStyle();
	var wrapper, pop_sfx, score, swch_btn;

	// start balloonz
	window.Balloonz = new function () {
		...
		this.start = function () {
			...
			if ( !this.isSet ) {
				...
				pop_sfx    = makeSFX();
				score      = new Score();
				swch_btn   = new SwitchBtn();
				this.isSet = true;
			}
			if ( !swch_btn.getState() ) return;

			this.balloonz    = [];
			var balloon_imgs = [
				'assets/b01.png',              // red
				'assets/b02.png',              // yellow
				'assets/b03.png',              // green
				'assets/b04.png',              // blue
				'assets/b05.png'               // purple
			];
			...
			for ( var i = 0; i < balloon_imgs.length * num_of_each; i++ ) {
				...
				this.balloonz.push( btmp );
			}
			//console.log( this.balloonz );
		};

		this.stop  = function () {
			for ( var i in this.balloonz )
				this.balloonz[ i ].destroy();
			this.balloonz = [];
		};
	...
	function makeSFX() {
		...
		return wrapper.appendChild( sfx_obj );
	};
	...
	function setStyle() {
		...
		lnk_obj.href = 'assets/balloonz.css';
	...
	function Balloon( imgsrc, bsize ) {
		this.size  = bsize;
		this.mouse = new Mouse( this, 'assets/parachute-mouse.gif' );
	...
	Balloon.prototype.addToPage = function ( imgsrc ) {
		...
		this.obj              = wrapper.appendChild( bal_obj );
	};
	...
	Balloon.prototype.animate = function () {
		...
		if ( this.y < -this.size * 3 ) this.reset(); // got to top
		this.update();                  // update div
		var _this = this;
	...
	Mouse.prototype.addToPage = function ( imgsrc ) {
		...
		this.obj                = wrapper.appendChild( mouse_obj );
	};
	...
	SwitchBtn.prototype.addToPage = function () {
		var imgsrc        = 'assets/btn_icon.png';
		...
		this.obj          = wrapper.appendChild( btn_obj );
